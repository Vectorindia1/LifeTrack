...
### 2026-08-29 — Home screen widget ("Today"), built with Glance
Added by user request. See PRD 7.11.
- **Library: Jetpack Glance** (`androidx.glance:glance-appwidget:1.2.0`), not classic RemoteViews — declarative, Kotlin-first, matches the rest of the app. Its AAR metadata requires `minCompileSdk=35`, comfortably under this project's 36 ceiling; verified by reading the AAR before adding the dependency, the same discipline used for every other library this session (see the Vico and toolchain entries).
- **Glance's composables are a genuinely different, smaller API from Jetpack Compose** — `androidx.glance.layout.Column`/`Row`, `androidx.glance.text.Text`, `androidx.glance.Button`, `GlanceModifier` (not `Modifier`), and no access to the app's `MaterialTheme` (the widget defines its own two colors in `WidgetColors`). Every signature used was confirmed by extracting the AAR's `classes.jar` and running `javap` — do not assume a full-Compose API is available inside a Glance composable.
- **Padding/size modifiers have both a `Dp` overload and a plain `Int` overload** (the `Int` one is interpreted as dp) — `GlanceModifier.padding(12)` is correct and simpler than constructing a `Dp`. Confirmed via `javap` after an earlier draft's `dp()` helper turned out to silently ignore its argument and always return the same value — caught before it ever reached a build.
- **`SizeMode.Single`** — one fixed layout regardless of resize, chosen because this widget's content doesn't need to reflow at different sizes. `SizeMode.Responsive`/`Exact` exist for widgets that do.
- **Freshness model:** the widget does a one-shot read (`Flow.first()`) each time it's asked to render, not a live subscription — Glance widgets render once per update, so there is nothing to gain from staying subscribed. Explicit `TodayWidget.refresh(context)` calls happen from (a) the dashboard's habit-toggle and water quick-add handlers, and (b) the widget's own `AddWaterAction` and the water-reminder notification's action buttons, so the common paths update within a second or two. The `updatePeriodMillis` in `today_widget_info.xml` (30 minutes, the OS's practical floor) is a background-only fallback for anything else, not the primary refresh path.
- The `+250ml` button's `ActionCallback` (`AddWaterAction`) writes to `WaterRepository` and calls `refresh()` itself — fully self-contained in the widget package, no ViewModel or Activity involved.

### 2026-08-29 — Interval water reminder: a deliberate, documented exception to "one notification"
Added by explicit user request — "set it like after every 60 min, so we cud set that and alarm wud ring." See PRD 7.12. Worth restating plainly since it looks, at a glance, like it contradicts a rule this project has enforced since session 0:
- **CLAUDE.md and PRD section 8 say notifications must be consolidated: never more than ~3 pushes/day, one daily digest, no per-tracker spam.** A 60-minute recurring water alert during waking hours is 12–14 pushes/day, which is a real, acknowledged departure from that rule — not an oversight. The tension was raised with the user before building; they wanted it anyway.
- **Kept structurally separate from the digest system**, not folded in: its own preference fields (`waterReminderEnabled`, `waterReminderIntervalMinutes`), its own notification channel (`lifetrack_water_reminder`, `IMPORTANCE_DEFAULT` so it can actually alert — the digest's channel is deliberately `IMPORTANCE_LOW`), its own notification id (1002, vs the digest's 1001), its own `WorkManager` unique work name, its own `WaterReminderWorker`/`WaterReminderScheduler`. **Do not merge these two systems** — a future session tempted to "clean up" by combining them would reintroduce spam into the one thing this app has otherwise kept quiet.
- **Off by default.** Turning it on is an explicit user action in Settings; the digest's existing water check is unaffected either way.
- **Not an `AlarmManager` exact alarm, despite the user saying "alarm wud ring".** Exact alarms on API 31+ need `SCHEDULE_EXACT_ALARM`/`USE_EXACT_ALARM`, both under Play Store policy scrutiny and intended for real alarm-clock apps. A `PeriodicWorkRequest` needs no special permission, reuses the same mechanism the daily digest already depends on, and gets the practical outcome the user asked for (an audible, recurring alert) without the permission risk. The "ring" is delivered by the notification channel's sound, not a true alarm-clock wake.
- **Reuses `DailyDigest.DAY_START`/`DAY_END` (08:00–22:00)** for its waking-hours gate rather than inventing a second window — one tested time range, two consumers.
- **Stops nagging once today's target is met** — checked inside the worker itself before posting, not left to the user to notice and disable.
- WorkManager's `PeriodicWorkRequest` has a hard 15-minute floor; both `PreferencesRepository.setWaterReminder` and `WaterReminderScheduler.apply` clamp to it independently, so a bad value can't sneak through either path.
- Notification actions (+small/+large) write via `WaterReminderActionReceiver`, a non-exported `BroadcastReceiver` using `goAsync()` to bridge its synchronous `onReceive` to the DAO's suspend insert — the same pattern would apply to any future notification action that needs to write to Room.

### 2026-08-29 — Migration v6: first NOT NULL column added via ALTER TABLE
- Every earlier `ALTER TABLE ADD COLUMN` in this project (`displayName`, `currencyLocaleTag`) added a **nullable** column, where SQLite defaults the existing rows to `NULL` automatically and no `DEFAULT` clause is needed.
- `waterReminderEnabled`/`waterReminderIntervalMinutes` are the first **NOT NULL** columns added after the fact, which SQLite requires a literal `DEFAULT` for. Used `DEFAULT 0` / `DEFAULT 60`, matching the Kotlin defaults (`false`/`60`).
- **Checked whether Room's migration validation compares that default value against anything — it does not.** The exported schema JSON (`app/schemas/.../6.json`) records `notNull` but no `defaultValue` key at all for this Room version (2.8.4); validation compares name/type/notNull/pk only. So the SQL default only has to satisfy SQLite's own requirement for adding a NOT NULL column, not match some Room-recorded expectation. Confirmed by inspecting the exported JSON directly rather than assuming.

### 2026-08-29 — Full backup/restore, added by user request
The safety net for "I updated the app and my data is gone" — a single JSON file covering all 12 tables, exportable and re-importable from Settings.
- **`backup/data/BackupDao.kt`** is one Dao spanning every table (get-all/insert-all/clear per table), deliberately separate from the 8 per-feature DAOs rather than scattering "get all rows" methods across them.
- **`backup/data/BackupCodec.kt`** hand-writes JSON mapping for each entity — no serialization library. Matches how `Converters.kt` already hand-writes Room's own type conversion; dates/times use the same representations as Room (`LocalDate`/`LocalTime` as their ISO `toString()`, `Instant` as epoch millis), so there is exactly one date/time convention across this codebase's persistence and its backup format, not two.
- **Import always replaces, never merges.** Merging would mean reconciling two independently-grown ID histories, a much harder problem this single-user app has no need to solve. The whole clear-then-insert sequence runs inside one `db.withTransaction { }`, and the import file is fully parsed *before* anything is cleared — a malformed or incompatible file changes nothing.
- **`EXPORT_FORMAT_VERSION`** is checked on import and mismatches are rejected outright with a plain-language error, rather than attempting a partial/lenient import. Bump it only when the JSON *shape* changes incompatibly.
- **Uses Storage Access Framework** (`ActivityResultContracts.CreateDocument`/`OpenDocument`) for the file picker, not raw file paths or a storage permission — the modern, scoped-storage-correct way to let a user pick where a file goes, and it needs no manifest permission at all.
- **`org.json` cannot be unit-tested as a plain JVM test in this project.** Verified empirically (not assumed): `org.json.JSONObject`/`JSONArray` are stubbed in the `android.jar` used for `testDebugUnitTest` and throw `RuntimeException` on any real call — this is different from most `android.*` stubs, which only throw for methods Robolectric doesn't cover; `org.json` throws unconditionally outside a real Android runtime or Robolectric. **`BackupCodec`/`BackupRepository` could not get the automated test coverage every other piece of business logic in this app has** as a result — verified only by careful manual review of every field mapping against the entity definitions, not by execution. If this ever needs real test coverage, the options are Robolectric (a real new dependency) or rewriting the codec on a JVM-native JSON approach (e.g. `kotlinx.serialization`) instead of `org.json` — neither was judged worth it for this one feature.

### 2026-08-29 — v1.0.0 crashed on every fresh install; root cause and the fix (v1.0.1)
A user report of "app opens then closes immediately" turned out to be **100% reproducible on any fresh install** — not intermittent, not device-specific. Root cause, found from a real device's logcat rather than guessed:

```
android.database.sqlite.SQLiteConstraintException: NOT NULL constraint failed: app_preferences.waterReminderEnabled
	at ... LifeTrackDatabase$Companion$SeedCallback.onCreate(LifeTrackDatabase.kt:128)
```

- **The bug:** when the session-12 water-reminder feature added `waterReminderEnabled`/`waterReminderIntervalMinutes` as `NOT NULL` columns (migration v5→v6), the `MIGRATION_5_6.migrate()` `ALTER TABLE` statements correctly included SQL-level `DEFAULT 0`/`DEFAULT 60` clauses (SQLite requires one for a NOT NULL column added after the fact — see the entry above this one). **But `SeedCallback.onCreate()`'s hardcoded `INSERT INTO app_preferences (...)` statement — the path a brand-new install uses instead of any migration — was never updated to include the two new columns.** A Kotlin default value (`= false`, `= 60` in the data class) is *not* a SQL `DEFAULT` clause; it only fills in the value when *Kotlin code* constructs the object, not when raw SQL inserts a row. With those two columns omitted from the INSERT's column list, and no SQL-level default because the entity-generated `CREATE TABLE` for a NOT NULL column has none unless `@ColumnInfo(defaultValue = ...)` is explicitly used, SQLite had nothing to put there and rejected the row outright.
- **Why the migration chain looked completely correct despite this:** it *was* completely correct. This bug lived entirely in `onCreate()`/`SeedCallback`, the fresh-install path, which is structurally separate from every migration. A real SQLite replay of the full v1→v6 migration chain (built from Room's own exported schema at each step) passed with zero mismatches — that check was accurate, it just wasn't checking the code path that was actually broken. **Lesson: every time a NOT NULL column is added, check both places it needs a value — the migration's `ALTER TABLE ... DEFAULT`, *and* `SeedCallback.onCreate()`'s explicit INSERT — not just the one that happens to be top of mind.** The existing seeded columns (`waterIncrementSmallMl` etc.) were already listed in that INSERT from session 10; only the two newest ones, added at the very end of a very long session, were missed.
- **The fix:** added `waterReminderEnabled`/`waterReminderIntervalMinutes` to `SeedCallback`'s INSERT, matching the migration's defaults (`0`/`60`) exactly.
- **How this was actually verified — the first real on-device testing this entire project has had:** the user connected a physical device over USB partway through diagnosing this. Confirmed live: (1) a clean install no longer crashes, (2) Settings' "Send a test notification now" button posts a real, correctly-formatted notification to the system — visible in the notification shade, channel `lifetrack_daily_digest`, content `"0 of 2000 kcal logged"` matching exactly what the digest logic should show for a freshly-seeded, empty-data install. This is the first time any part of this app has been confirmed working by *seeing it run*, rather than by compiling, unit-testing pure logic, or replaying schemas — everything before this was verified as far as those methods can reach, and no further.
- Shipped as **v1.0.1** (versionCode 2) — v1.0.0's GitHub release is left in place with its notes explaining it was superseded immediately, rather than silently overwriting the asset.
